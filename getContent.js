...
async function extractPMCID(pmid) {
  const url = `https://pmc.ncbi.nlm.nih.gov/tools/idconv/api/v1/articles/?ids=${pmid}&format=json`;

  let retries = 2;
  while (retries > 0) {
    try {
      const { data } = await axiosGetWithRetry(url, { timeout: 4000 });
...
async function fetchFullTextXML(pmcid) {
  const url = `https://pmc.ncbi.nlm.nih.gov/api/oai/v1/mh/?verb=GetRecord&identifier=oai:pubmedcentral.nih.gov:${pmcid}&metadataPrefix=pmc`;
...
async function bulkExtractPMCID(pmids) {
  ...
  const url = `https://pmc.ncbi.nlm.nih.gov/tools/idconv/api/v1/articles/?ids=${pmids.join(',')}&format=json&versions=no`;
...
async function bulkFetchAbstracts(pmids) {
  ...
  while (retries > 0) {
    try {
      const response = await axiosGetWithRetry(url, { timeout: 15000 });
      data = response.data;
      break;
    } catch (err) {
      const isNetworkError = !err.response || err.code === 'ECONNRESET' || err.message.includes('socket hang up') || err.message.includes('timeout');
...
async function preFetchBatchArticles(articlesBatch) {
  ...
  // Fetch full text PMC XMLs sequentially with a small delay to avoid rate-limiting or socket hangs
  for (const { pmid, pmcid } of pmcidsToFetch) {
    try {
      const fullTextXML = await fetchFullTextXML(pmcid);
    ...
    // 300ms delay to respect PMC OAI-PMH rate limits
